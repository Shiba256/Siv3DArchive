...
ファイルアーカイブを作成するには、`Archive::Setting`関数を使用し、アーカイブの出力先、暗号化用の鍵、AESのモードを指定します。暗号化用の鍵には`AES::Key`を使用します。AESのモードは`AES::Mode`で指定でき、`CBC`,`CTR`,`CFB`,`ECB`の4種類が実装されています。省略時はデフォルトで`CTR`モードが使用されます。
